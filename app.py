...
from models_new import db, User, Order, OrderItem
...
def create_order():
    ...
    if request.method == "POST":
        ...
        voice_blobs = request.form.getlist("voice_note_blob[]")
        ...
        for i in range(len(product_names)):
            ...
            if i < len(voice_blobs) and voice_blobs[i]:
                try:
                    header, encoded = voice_blobs[i].split(",", 1)
                    binary_data = base64.b64decode(encoded)

                    filename = f"{uuid.uuid4()}.webm"

                    save_path = os.path.join("static/voice_notes", filename)
            ...
            # 🔥 SET VOICE SEPARATELY (THIS FIXES YOUR ERROR)
            item.voice_note = filename
...
def edit_order(order_id):
    ...
    if request.method == "POST":
        ...
        # 🔥 NEW: GET VOICE FILES
        voice_files = request.files.getlist("voice_note[]")

        for i in range(len(product_names)):

            # 🔥 HANDLE VOICE FILE
            voice_file = voice_files[i] if i < len(voice_files) else None
            filename = None

            if voice_file and voice_file.filename != "":
                filename = secure_filename(voice_file.filename)
                voice_file.save(os.path.join("static/voice_notes", filename))
